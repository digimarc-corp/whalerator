...
namespace Whalerator.Client
{
    public abstract class AuthHandler : IAuthHandler
    {
        ...
        public virtual string GetToken(AuthenticationHeaderValue authenticateHeader)
        {
            ...
            return GetToken(dict["realm"], dict.Where(v => !v.Key.Equals("realm")).Select(v => (v.Key, v.Value)).ToArray());
        }

        public virtual string GetToken(string realm, string service, string scope)
        {
            return GetToken(realm, (nameof(service), service), (nameof(scope), scope));
        }

        public abstract string GetToken(string realm, params (string name, string value)[] values);
